...
module.exports = {
  ...
  var current_date=moment().format("YYYY-MM-DD")
                    ...
                    if (err) {
                        ...
                        else
                        {
                            if(updateResults.affectedRows > 0)
                            {
                                ...
                                var update_sgh_null = "update  hrm_employee_salary_details set shg_deduction='0' where emp_id="+emp_id+"";
    ...
    EmployeeforgotPassword: (req) => {
        ...
        var query = "SELECT ed.email,um.id,IFNULL(ed.firstname,'') as firstname,IFNULL(ed.middlename,'') as middlename,IFNULL(ed.lastname,'') as lastname FROM hrm_employee_details as ed inner join hrm_user_master as um  on um.id=ed.emp_id and um.role_id!=1 WHERE (ed.email = ? or um.emp_id = ?) AND um.status = '1'";
        sql.query(query, [email, email], function (err, userData) {
            if (err) {
                console.log(err);
                deferred.resolve({ status: 0, message: 'Getting new password failed' });
            } else {
  ...
  var hashdata=new Buffer(currentdate)
  var randomstring=hashdata.toString('base64');
  console.log("hashdate",randomstring)
  var link =`<a href="http://${hostname}:3000/reset_password/${empid}/${randomstring}">link </a>`
    ...
    resetPassword: (req) => {
        ...
        var hashed_date=req.body.hashed_date
        var new_password = req.body.new_password;
        var confirm_password = req.body.confirm_password
        let buff=new Buffer(Key,'base64')
        var decoded=buff.toString('ascii');
        let buffer=new  Buffer(hashed_date,'base64')
        var decoded=buff.toString('ascii');        
        var decoded_date=buffer.toString('ascii');
        let currentdate=moment.utc().format("YYYY-MM-DD")
        var comporedate= moment(currentdate).diff(decoded_date,'days');
    if(comporedate==0)
    {
    ...
    {
        deferred.resolve({status:0,message:"Your link has been expired"})
    }
