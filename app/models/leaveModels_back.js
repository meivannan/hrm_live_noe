...
module.exports = {
    ...
    leavePermission: (req) => {
        ...
        var CheckingQuery = "select from_date,to_date from hrm_leave_request where leave_type_id=" + leave_type + "  and emp_id=" + emp_id; //and YEAR(from_date) = " + current_year + "
                                    ...
                                    sql.query(approvelQuery, function (err, masterData) {
                                        if (err) {
                                            ...
                                            if (masterData) {
                                                ...
                                                var noticationQuery = "INSERT INTO hrm_notication(from_emp_id,to_role_id,filter,message) VALUES (" + emp_id + ",0,'leave','" + message + "') "
                                                sql.query(noticationQuery, function (err, noticationdata) {
                                                    if (err) {
                                                        console.log(err);
                                                        deferred.resolve({ status: 0, message: "Something Went Wrong" });
...
return deferred.promise;
    ...
    leaveHistory: (leave_type, from_date, to_date, status, emp_id) => {
        ...
        if(from_date != ''){
            condition += (condition != '') ? ' and from_date = "'+from_date+'" ' : ' from_date = "'+from_date+'" '
        }
...
sql.query(checkingQuery,function(err,checkdata)
{
    ...
    dashbordlist: (company_id) => {
        ...
        sql.query(leave_type, function (err, data) {
            if (err) {
                ...
                var permission_query = "select count(ld.emp_id) as count from hrm_leave_request as ld  inner join hrm_user_master as um on ld.emp_id=um.id where ld.leave_type='" + leave_type + "'  and um.role_id!=1 and um.company_id=" + company_id + " and from_date=CURDATE()";
                sql.query(permission_query, function (err, result) {
                    ...
                    if (err) {
                        ...
                        var employee_count = "select count(id) as total_count, GROUP_CONCAT(emp_id) as emp_ids from hrm_user_master where status=1 and role_id!=1 and company_id=" + company_id + "";
                        sql.query(employee_count, function (err, employeedata) {
                            if (err) {
                                ...
                                if (employeedata.length > 0) {
                                    ...
                                    var attendence_query = "select   count(ad.emp_id) as present_count from hrm_employee_attendence as ad inner join hrm_user_master as um  on um.id=ad.emp_id where um.role_id!=1 and um.company_id=" + company_id + " and um.status=1 and DATE(check_in)=DATE(CURDATE())"
                                        ...
                                        if (err) {
                                            ...
                                            sql.query(leave_request, function (err, new_count) {
                                                if (err) {
                                                    ...
                                                    var newemployeeQuery="select COUNT(em.emp_id) as newemployee_count from hrm_employee_details as em inner join hrm_user_master  as um on um.id=em.emp_id where um.status=1 and um.role_id!=1 and um.company_id="+company_id+" and MONTH(em.joined_date)=MONTH(CURDATE())"
                                                    sql.query(newemployeeQuery,function(err,newjoindata)
                                                    {
                                                        ...
                                                        else
                                                        {
                                                            ...
                                                            sql.query(employee_countquery,function(err,employee_data){
                                                                if(err)
                                                                {
                                                                    console.log(err)
                                                                }
                                                                    ...
                                                                    deferred.resolve({ status: 1, message: "Dashboard successfully", present_count: attendence_count, permssion_count: permission_count, employee_count: employee_count, absent_count: absent_count, leave_new_request_count: leave_new_request_count, percenatge: percentage,newjoined_count:newjoined_count,perment_count:perment_count,tempory_count:tempory_count })
                                                                }
                                                            })
            ...
            if (err) {
                ...
                leaveType.forEach(leave => {
                  ...
                  let details = {
                        ...
                        remaining_days:(leaveTypeObject[leave.l_id] - currentTaken),
                        duration:'Days'
                    }
                    list.push(details);
                });
    ...
    Exportleave_details: (company_id) => {
        var deferred = q.defer();
        var query = "Select  lr.*,um.emp_id as empid,CONCAT(IFNULL(em.firstname,''),'',IFNULL(em.middlename,''),'',IFNULL(em.lastname,'')) as employeename FROM hrm_leave_request as lr inner join  hrm_employee_details  as em  on lr.emp_id=em.emp_id inner join hrm_user_master as um  on um.id=lr.emp_id WHERE um.status='1' and um.company_id=" + company_id + " group by lr.leave_id  ";
        sql.query(query, function (err, user) {
    ...
    notificationList: (company_id) => {
        var deferred = q.defer();
        var query = "Select  n.*,um.emp_id as empid,CONCAT(IFNULL(em.firstname,''),'',IFNULL(em.middlename,''),'',IFNULL(em.lastname,'')) as employeename FROM hrm_notication as n inner join  hrm_employee_details  as em  on n.from_emp_id=em.emp_id inner join hrm_user_master as um  on um.id=.n.from_emp_id WHERE um.status='1' and um.company_id=" + company_id + " and  n.isRead=0 and MONTH(n.created_on) = MONTH(CURRENT_DATE()) AND YEAR(n.created_on) = YEAR(CURRENT_DATE())  ";
        sql.query(query, function (err, user) {
            if (err) {
                console.log(err);
                deferred.resolve({ status: 0, message: 'Something  Went Wrong' });
    ...
    updateNotification: (notification_id) => {
        ...
        sql.query(checkQuery,function(err,data)
        {
            ...
            else
            {
                if(data.length > 0)
                {
                    var query = "Update hrm_notication SET isRead=1 where id="+notification_id+"";
                    console.log(query)
                     sql.query(query, function (err, user) {
                         if (err) {
                             console.log(err);
